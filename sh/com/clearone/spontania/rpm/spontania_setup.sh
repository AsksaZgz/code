...
v_packagesCheck="sudo postgresql-libs-8 postgresql-8 postgresql-odbc-08 postgresql-server-8 cronie"
...
if [ $? -eq 0 ]
then
  v_numLines=`ps ax | grep postmaster | grep /usr/bin | wc -l`
  if [ 0 -eq $v_numLines ]
  then
...
install -c -o root /usr/local/Spontania/lib/* /usr/local/lib
cp -r /usr/local/Spontania/lib/libcryp.tar.gz /usr/local/lib/.
cd /usr/local/lib
...
ldconfig >> $v_logFile 2>> $v_logFile
cd /usr/lib # rpm 006					
# rpm 005
echo -e "Linking libodbc --------------------------------------------------------------------------------" 	>> $v_logFile 2>> $v_logFile			
ln -s libodbc.so.2 libodbc.so.1																				>> $v_logFile 2>> $v_logFile			
echo -e "Linking libodbc --------------------------------------------------------------------------------/" >> $v_logFile 2>> $v_logFile			
echo -e "Linking libcurl --------------------------------------------------------------------------------" 	>> $v_logFile 2>> $v_logFile			
ln -s libcurl.so.4 libcurl.so.3 																			>> $v_logFile 2>> $v_logFile										
echo -e "Linking libcurl --------------------------------------------------------------------------------/" >> $v_logFile 2>> $v_logFile	
echo -e "Linking libssl --------------------------------------------------------------------------------" 	>> $v_logFile 2>> $v_logFile	
ln -s libssl3.so libssl.so.4  																				>> $v_logFile 2>> $v_logFile	
echo -e "Linking libssl --------------------------------------------------------------------------------/" 	>> $v_logFile 2>> $v_logFile	
# rpm 005 /

echo -e "Installing server ..."
echo -e "Installing server ..." >> $v_logFile
cd /usr/local/Spontania/server
tar -zxvf /usr/local/Spontania/server/webconferencebase.tar.gz
install -c -o root /usr/local/Spontania/server/webconferencebase /usr/local/bin/
chmod 0755 /usr/local/bin/webconferencebase

###########################################################################
#INSTALANDO FICHERO DE CONFIGURACION DEL SERVIDOR
###########################################################################
echo -e "Installing server configuration file ..."
echo -e "Installing server configuration file ..." >> $v_logFile
install -c -o root /usr/local/Spontania/server/webconference-config.xml /etc/webconference-config.xml
...
/usr/local/Spontania/license/SerialGenerator > /tmpwebconference/serial.gen 2>> $v_logFile
# rmp 004 ################################################################
...
# rpm 004 / 
service postgresql stop >> $v_logFile 2>> $v_logFile
echo " kernel.shmmax = 536870912" >> /etc/sysctl.conf 2>> $v_logFile
sysctl -p >> $v_logFile 2>> $v_logFile
#POR SEGURIDAD CONFIGURAMOS EL ARRANQUE DE POSTGRESQL AUNQUE ES POSIBLE
#QUE YA ESTE
chkconfig postgresql on 2>> $v_logFile
echo -e "Preparing database ..."
echo -e "Preparing database ..." >> $v_logFile
...
sed -e 's/ident/trust/g' /var/lib/pgsql/data/pg_hba.conf > /var/lib/pgsql/data/aux.xml 2>> $v_logFile
# rpm 005/
mv -f /var/lib/pgsql/data/aux.xml /var/lib/pgsql/data/pg_hba.conf 2>> $v_logFile
echo "host    all         all         $v_ipOne         255.255.255.255   trust" >> /var/lib/pgsql/data/pg_hba.conf
if [ "$v_ipOne" !=  "$v_ipTwo" ]
then
  echo "host    all         all         $v_ipTwo         255.255.255.255   trust" >> /var/lib/pgsql/data/pg_hba.conf
fi
echo -e "Starting postgresql ..."
echo -e "Starting postgresql ..." >> $v_logFile
service postgresql start >> $v_logFile 2>> $v_logFile
sleep 30
...
install -c -o root /usr/local/Spontania/server/openconfiguration.tgz /openconfiguration.tgz                    >> $v_logFile 2>> $v_logFile		
cd /
/bin/tar -xzvf /openconfiguration.tgz                                                                          >> $v_logFile 2>> $v_logFile		
